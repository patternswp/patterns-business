 ...
 * @subpackage Patterns_Business/Patterns_Business_Info
 */
...
class Patterns_Business_Info {
	...
	public function add_theme_menu() {
		$this->hook_suffix[] = add_theme_page( esc_html__( 'Theme Info', 'patterns-business' ), esc_html__( 'Theme Info', 'patterns-business' ), 'edit_theme_options', PATTERNS_BUSINESS_THEME_NAME, array( $this, 'info_screen' ) );
	}
	...
	public function enqueue_resources( $hook_suffix ) {
		...
		$unique_id = PATTERNS_BUSINESS_THEME_NAME . '-info';
		...
		$deps_file = PATTERNS_BUSINESS_PATH . 'build/admin/info/info.asset.php';
		...
		wp_enqueue_script( $unique_id, PATTERNS_BUSINESS_URL . 'build/admin/info/info.js', $dependency, $version, true );

		wp_enqueue_style( $unique_id, PATTERNS_BUSINESS_URL . 'build/admin/info/info.css', array(), $version );
		wp_style_add_data( $unique_id, 'rtl', 'replace' );

		/* Localize */
		$localize = apply_filters(
			'patterns_business_info_localize',
			array(
				'version'  => $version,
...
/**
 * Return instance of Patterns_Business_Info class
 *
 * @since 1.0.0
 *
 * @return Patterns_Business_Info
 */
function patterns_business_info() { //phpcs:ignore
	return Patterns_Business_Info::instance();
}
patterns_business_info()->run();
